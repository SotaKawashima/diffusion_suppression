...
from difftools.optimization.minimization import *
...
def sample1():
    ...
    S, hist = sim_min(adj, sample_size, pop_list, interest_list, assum_list, SeedSet_F, k_T, prob_map)

    path_name = wf.set_path('minimization')
...
def sample2():
        ...
        for pop_list in pop_combo:
            ...
            for i, x in enumerate(hist):
                df = df.append({
                    ...
                    'infl_sprd_F': x[InfoType_F].min() / n,
                    'infl_sprd_T': x[InfoType_T][x[InfoType_F].argmin()] / n
                }, ignore_index = True)
    ...
    print(df)
    path_name = wf.set_path('minimization_2')
...
def sim_min(adj, sample_size, pop_list, interest_list, assum_list, SeedSet_F, k_T, prob_map):
    S, hist = minimization(None, k_T, sample_size, adj, SeedSet_F, prob_map, pop_list, interest_list, assum_list)
    print('\nS\n', S)
    # print('\nhist\n', hist)
